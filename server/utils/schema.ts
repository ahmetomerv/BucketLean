...
export const optimizationJobs = sqliteTable('optimization_jobs', {
  ...
  status: text('status', { enum: ['queued', 'running', 'completed', 'needs_attention'] }).notNull(),
...
export const optimizationItems = sqliteTable('optimization_items', {
  ...
  optimizedSize: integer('optimized_size'),
  savedPercent: integer('saved_percent'),
  status: text('status', { enum: ['pending', 'downloading', 'processing', 'uploading', 'completed', 'skipped', 'failed', 'needs_attention'] }).notNull(),
  error: text('error'),
