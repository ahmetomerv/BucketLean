...
export default defineEventHandler(async (event) => {
  const body = await readBody<{ prefix?: unknown, bucketId?: unknown }>(event)
  ...
  const activeJob = getDatabase().select({ id: optimizationJobs.id }).from(optimizationJobs)
    .where(and(eq(optimizationJobs.bucketId, bucketId), inArray(optimizationJobs.status, ['queued', 'running', 'paused', 'needs_attention']))).get()
  if (activeJob) throw createError({ statusCode: 409, statusMessage: 'Resolve the optimization job before scanning' })
  try {
    const result = enqueueScan(prefix, bucketId)
