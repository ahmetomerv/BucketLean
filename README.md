...
2. In the dashboard's job controls, filter by key prefix and **Minimum original size (MiB)** (a dropdown with 1 through 20 MiB, default 1 MiB). The eligible count and object list use the same filters. Select JPEGs in **Scan results** with their checkboxes, by clicking eligible rows, or with **Select eligible on this page**. Selections persist across pages; the selected count appears in the Scan results heading and on **Start job**. Changing bucket, scan, prefix, size, or status clears the selection. Up to 5,000 objects can be selected per job. Use **Show preview** in the last column to load a read-only 72 px thumbnail on demand without changing selection; each preview reads that object once from R2. Only JPEGs confirmed as not previously optimized can be selected. Objects with unknown metadata are excluded. Clicking **Start job** opens a confirmation window showing the bucket, selected count, and backup policy; canceling leaves the selection unchanged. Selected keys are validated and inserted into one SQLite transaction in batches of 100 after confirmation.
...
Check a few optimized images in every application that uses the bucket. The optimizer replaces R2 objects directly; it does not update another application's database, displayed file sizes, thumbnails, or caches. If an application still shows old information, use its refresh, reindex, or reprocess action after checking what that action changes. For example, a ChronoFrame user found that running **Reprocess** in its dashboard refreshed displayed image sizes after an optimization job. Confirm that the viewer and downloads use the optimized images before treating the integration as complete.
...
Every replacement requires a verified original backup and restore manifest; there is no backup creation option in the dashboard. The **Delete backup after successful optimization** checkbox is off by default; its info tooltip explains the restore impact. It is a per-job setting (`deleteBackupAfterOptimization: true` in `POST /api/jobs`). The worker creates and verifies the backup and manifest before replacing a source. It then verifies the optimized source, records `cleanup_pending` in SQLite, rechecks the source and backup, deletes the backup, verifies it is absent, deletes the manifest, verifies it is absent, and marks the item `completed`. This saves the retained backup storage but removes the per-image restore copy. The optimized object at the original key is never deleted by this option.
