...
2. In the dashboard's job controls, filter by key prefix and **Minimum original size (MiB)** (default 1 MiB; presets of 3, 5, and 8 MiB; custom values from 1 to 50 MiB). The eligible count and object list use the same filters, while the table's status filter only changes what is displayed. Use **Show preview** on a listed JPEG to load a read-only 72 px thumbnail on demand; each preview reads that object once from R2. Only JPEGs confirmed as not previously optimized enter a job. Objects with unknown metadata are excluded. Candidate keys are read and inserted into one SQLite transaction in batches of 100, so job creation does not hold the entire candidate list in memory.
