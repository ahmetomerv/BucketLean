...
2. In **Optimize eligible JPEGs**, set the key prefix and **Minimum original size (MiB)**, then inspect the eligible count and object list. The default is 1 MiB. Choose the 3, 5, or 8 MiB preset, or enter any value from 1 to 50 MiB, including decimals. Press **Show preview** on a listed JPEG to load a small thumbnail before starting a job; previews are read-only and downloaded on demand. The scan itself still reads its selected bucket or scan prefix. The job includes only scanned JPEGs at or above this size, with known metadata and no `image-optimizer-version` marker. The object table defaults to **All**, so seeing an image there does not mean it is eligible.
3. Create a job for the chosen prefix and size. Balanced quality 82 and a minimum 15% saving are the defaults. A verified original backup is mandatory before replacement. The optional deletion checkbox is off by default; selecting it removes the backup and manifest only after the optimized source is verified, so you lose that image's restore copy.
