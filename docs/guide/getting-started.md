...
1. In the combined bucket and scan panel, select a bucket and enter a narrow scan prefix containing one or two known JPEGs, then use **Start scan** at the bottom of the panel. The button shows a spinner while the scan is queued or running. Scanning reads R2 but does not change objects.
