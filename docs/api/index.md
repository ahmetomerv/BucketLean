...
```sh
curl --user operator http://localhost:3000/api/jobs
```
...
1. `POST /api/scan` with a narrow prefix. A scan reads R2 only.
2. Poll `GET /api/overview` until `scan.status` is `completed`; check `metadataErrorCount` as well.
3. Inspect `GET /api/objects` with the same prefix and size filter.
4. `POST /api/jobs` to start a job. This can replace qualifying JPEGs after backup and verification.
